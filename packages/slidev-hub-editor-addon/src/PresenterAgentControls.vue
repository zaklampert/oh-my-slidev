...
const { currentSlideNo, currentSlideRoute, isPresenter } = useNav()
...
<template>
  <template v-if="isPresenter">
    <IconButton :title="panelOpen ? 'Hide AI panel' : 'Show AI panel'" @click="panelOpen = !panelOpen">
      <div class="i-carbon:bot" />
    </IconButton>

    <Teleport to="body">
      <div v-if="panelOpen" class="fixed inset-y-0 right-0 z-[1000] w-[420px] max-w-[92vw] bg-[#0e1117] text-gray-100 border-l border-white/10 shadow-2xl">
        <div class="h-full grid grid-rows-[max-content_1fr]">
          <header class="flex items-start justify-between gap-3 border-b border-white/10 px-4 py-3">
            ...
            <button class="rounded border border-white/10 px-2 py-1 text-xs" @click="panelOpen = false">
              Close
            </button>
          </header>
